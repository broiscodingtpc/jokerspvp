...
const duel_1 = require("../lib/duel");
const wallet_1 = require("../lib/wallet");
const tokenChecker_1 = require("../lib/tokenChecker");
...
const activeGroupChats = new Set();
// Store last announcement message IDs for each chat to delete old ones
const lastAnnouncementMessages = new Map();
// Animated duel function
async function playDuelAnimation(chatId, player1Name, player2Name, amount, onComplete) {
...
async function announceActiveDuels() {
    try {
        const activeDuels = await db_1.db.getAllPendingDuels();
...
`;
        ...
        activeDuels.forEach((duel, index) => {
...
`;
        ...
        for (const chatId of activeGroupChats) {
            try {
                // Delete previous announcement message if it exists
                const lastMessageId = lastAnnouncementMessages.get(chatId);
                if (lastMessageId) {
                    try {
                        await bot.deleteMessage(chatId, lastMessageId);
                        console.log(`Deleted old announcement message ${lastMessageId} in chat ${chatId}`);
                    }
                    catch (deleteError) {
                        console.log(`Could not delete old announcement message ${lastMessageId} in chat ${chatId}:`, deleteError);
                    }
                }
                // Send new announcement
                const newMessage = await bot.sendPhoto(chatId, './assets/images/duels-banner.png', {
                ...
                lastAnnouncementMessages.set(chatId, newMessage.message_id);
                console.log(`Sent new announcement message ${newMessage.message_id} to chat ${chatId}`);
            }
            catch (error) {
                ...
                try {
                    ...
                    const newMessage = await bot.sendMessage(chatId, announcementText, {
                        parse_mode: 'Markdown',
                        reply_markup: {
                            inline_keyboard: inlineKeyboard
                        },
                    ...
                    // Store the new message ID for future deletion
                    lastAnnouncementMessages.set(chatId, newMessage.message_id);
                    console.log(`Sent fallback announcement message ${newMessage.message_id} to chat ${chatId}`);
                }
                catch (fallbackError) {
                    ...
                    lastAnnouncementMessages.delete(chatId);
                }
            }
        }
    }
...
bot.onText(/\/pvp/, async (msg) => {
    ...
    if (!user) {
        const { wallet } = await wallet_1.walletManager.generateWallet(telegramId, username);
    ...
    if (msg.chat.type === 'private') {
        ...
        try {
            ...
            const tokenBalance = await wallet_1.walletManager.getTokenBalance(telegramId);
            const stats = await duel_1.duelManager.getUserStats(telegramId);
...
`;
        ...
        const leaderboard = await duel_1.duelManager.getLeaderboard();
        let leaderboardText = '';
    ...
    try {
        switch (data) {
            case 'register_wallet':
                ...
                break;
            case 'get_wallet_private':
                await handleGetWalletPrivate(telegramId, chatId, callbackQueryId);
                break;
            case 'start_duel':
...
async function handleRegisterWallet(telegramId, groupChatId, callbackQueryId) {
    try {
...
// Handle get wallet private (new function)
async function handleGetWalletPrivate(telegramId, groupChatId, callbackQueryId) {
    try {
        const user = await db_1.db.getUser(telegramId);
        if (!user) {
            await bot.answerCallbackQuery(callbackQueryId, { text: 'User not found' });
        ...
        await bot.answerCallbackQuery(callbackQueryId, { text: 'Opening private chat...' });
        try {
            // Send wallet information to private chat
            const connection = new web3_js_1.Connection(process.env.RPC_URL || 'https://api.mainnet-beta.solana.com');
            const walletPubkey = new web3_js_1.PublicKey(user.wallet);
            const solBalance = await connection.getBalance(walletPubkey);
            const tokenBalance = await wallet_1.walletManager.getTokenBalance(telegramId);
...
`;
            ...
            await bot.sendMessage(telegramId, walletMessage, {
                parse_mode: 'Markdown',
                reply_markup: privateKeyboard
            });
            // Send confirmation in group
            await bot.sendMessage(groupChatId, `✅ @${user.username || `User${telegramId}`} check your private chat for wallet details!`, { parse_mode: 'Markdown' });
        }
        catch (error) {
            console.log('Could not send private message, sending group instructions');
            // Fallback: send instructions in group
            const errorMessage = `
❌ *Private Chat Required*
...
`;
            const groupKeyboard = {
                inline_keyboard: [
                    [
                        { text: '🔐 Open Private Chat', url: `https://t.me/jokerspvpbot?start=wallet` }
                    ]
                ]
            };
            await bot.sendMessage(groupChatId, errorMessage, {
        ...
        console.error('Error in handleGetWalletPrivate:', error);
        await bot.answerCallbackQuery(callbackQueryId, { text: 'Error occurred' });
    }
}
// Handle start duel menu
async function handleStartDuel(telegramId, chatId, messageId, callbackQueryId) {
    const user = await db_1.db.getUser(telegramId);
    if (!user) {
...
async function handleDuelAmount(telegramId, chatId, amount, messageId, callbackQueryId) {
    try {
        const user = await db_1.db.getUser(telegramId);
        const username = user?.username || `User${telegramId}`;
        const result = await duel_1.duelManager.startDuel(telegramId, amount);
        if (result.success) {
            // Answer callback immediately
...
// Handle join duel
async function handleJoinDuel(telegramId, chatId, messageId, callbackQueryId) {
    try {
        const user = await db_1.db.getUser(telegramId);
        const username = user?.username || `User${telegramId}`;
        const result = await duel_1.duelManager.joinDuel(telegramId);
        if (result.success) {
            // Answer callback immediately
            await bot.answerCallbackQuery(callbackQueryId, { text: 'Joining duel...' });
            // Get duel info for display
            const pendingDuels = await db_1.db.getAllPendingDuels();
            if (pendingDuels.length > 0) {
                const duel = pendingDuels[0]; // Get the first available duel
                const player1User = await db_1.db.getUser(duel.player1);
                ...
                const completedDuel = await db_1.db.getDuel(duel.id);
...
async function handleJoinSpecificDuel(telegramId, chatId, messageId, callbackQueryId, duelId) {
    ...
    try {
        ...
        // Join the duel
        const result = await duel_1.duelManager.joinDuel(telegramId, duelId);
        ...
        if (result.success) {
            ...
            // Get the completed duel to see who actually won
            const completedDuel = await db_1.db.getDuel(duelId);
            if (!completedDuel || !completedDuel.winner) {
                console.log('Could not determine winner from completed duel');
                await bot.sendMessage(chatId, '❌ Error determining winner');
                return;
...
// Handle how to play
async function handleHowToPlay(chatId) {
    const message = `
...
// Handle back to menu
async function handleBackToMenu(telegramId, chatId, messageId) {
    // Get top 3 leaderboard
    const leaderboard = await duel_1.duelManager.getLeaderboard();
    let leaderboardText = '';
    if (leaderboard.length > 0) {
...
`;
                            ...
                            // Group chat - show interactive menu with leaderboard and tutorial
                            // Get top 3 leaderboard
                            const leaderboard = await duel_1.duelManager.getLeaderboard();
                            let leaderboardText = '';
                            if (leaderboard.length > 0) {
                                leaderboardText = '\n🏆 *TOP 3 WARRIORS*\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
                                leaderboard.slice(0, 3).forEach((entry, index) => {
                                    const uname = entry.username ? `@${entry.username}` : `User${entry.telegram_id}`;
                                    const emoji = index === 0 ? '🥇' : index === 1 ? '🥈' : '🥉';
                                    leaderboardText += `${emoji} *${uname}* — ${entry.wins}W/${entry.losses}L\n`;
                                });
                            }
...
📋 *How to Start:*
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1️⃣ Click "🔐 Get Your Wallet" below
2️⃣ Bot will send you wallet details in private
3️⃣ Send SPL tokens + 0.01 SOL for fees
4️⃣ Return here and use /duel or /join
...
⚡ *Active duels are announced every 3 minutes!*
`;
                            const menuKeyboard = {
                                inline_keyboard: [
                                    [
                                        { text: '🔐 Get Your Wallet', callback_data: 'get_wallet_private' },
                                        { text: '⚔️ Start Duel', callback_data: 'start_duel' }
                                    ],
                                    [
...
if (process.env.NODE_ENV === 'production') {
    tokenChecker_1.tokenChecker.startMonitoring();
}
// Cleanup old duels periodically
setInterval(async () => {
    await duel_1.duelManager.cleanupOldDuels();
}, 60000); // Every minute
...
bot.onText(/\/cancel_duel/, async (msg) => {
    ...
    try {
        const result = await duel_1.duelManager.cancelDuel(telegramId);
        if (result.success) {
...
bot.onText(/\/withdraw_sol (.+)/, async (msg, match) => {
    ...
    try {
        const result = await wallet_1.walletManager.withdrawSol(telegramId, amount, solAddress);
        if (result.success) {
...
bot.onText(/\/export_wallet/, async (msg) => {
    ...
    try {
        const result = await wallet_1.walletManager.exportWallet(telegramId);
        if (result.success) {
            const exportMessage = `
🔐 **WALLET EXPORT**
...
bot.onText(/\/join @(.+)/, async (msg, match) => {
    ...
    try {
        ...
        const targetUser = allUsers.find((user) => user.username && user.username.toLowerCase() === targetUsername.toLowerCase());
        if (!targetUser) {
            await bot.sendMessage(chatId, `❌ User @${targetUsername} not found.`);
            return;
        }
        // Check if target user has a pending duel
        const userPendingDuel = await db_1.db.getUserPendingDuel(targetUser.telegram_id);
        if (!userPendingDuel) {
            await bot.sendMessage(chatId, `❌ @${targetUsername} doesn't have a pending duel.`);
            return;
        }
        // Join the specific duel
        const result = await duel_1.duelManager.joinDuel(telegramId, userPendingDuel.id);
        if (result.success) {
            const player1Name = targetUser.username || `User${userPendingDuel.player1}`;
            // Get the completed duel to see who actually won
            const completedDuel = await db_1.db.getDuel(userPendingDuel.id);
            if (!completedDuel || !completedDuel.winner) {
                console.log('Could not determine winner from completed duel');
                await bot.sendMessage(chatId, '❌ Error determining winner');
                return;
...
bot.onText(/\/withdraw (.+)/, async (msg, match) => {
    ...
    try {
        ...
        const realBalance = await wallet_1.walletManager.getTokenBalance(telegramId);
...
`, { parse_mode: 'Markdown' });
        ...
        try {
            ...
            const signature = await wallet_1.walletManager.transferTokens(telegramId, solAddress, amount);
            if (signature) {
                ...
                await db_1.db.updateBalance(telegramId, newBalance);
                const successMessage = `
...
async function handlePrivateBalance(telegramId, callbackQueryId) {
    const user = await db_1.db.getUser(telegramId);
    if (!user) {
        await bot.answerCallbackQuery(callbackQueryId, { text: 'User not found' });
        return;
...
async function handlePrivateDeposit(telegramId, callbackQueryId) {
    const user = await db_1.db.getUser(telegramId);
    if (!user) {
        await bot.answerCallbackQuery(callbackQueryId, { text: 'User not found' });
        return;
...
async function handlePrivateWithdraw(telegramId, callbackQueryId) {
    try {
...
async function handlePrivateStats(telegramId, callbackQueryId) {
    const user = await db_1.db.getUser(telegramId);
    if (!user) {
        await bot.answerCallbackQuery(callbackQueryId, { text: 'User not found' });
        return;
    ...
    try {
        const stats = await duel_1.duelManager.getUserStats(telegramId);
        const balance = await duel_1.duelManager.getBalance(telegramId);
        const statsMessage = `
📊 *YOUR STATS*
...
async function handlePrivateCancelDuel(telegramId, callbackQueryId) {
    try {
...
async function handlePrivateWithdrawSol(telegramId, callbackQueryId) {
    try {
...
async function handlePrivateExportWallet(telegramId, callbackQueryId) {
    try {
